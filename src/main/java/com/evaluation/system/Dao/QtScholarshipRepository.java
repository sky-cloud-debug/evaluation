...
public interface QtScholarshipRepository extends JpaRepository<qtScholarship,String> {
    ...
    //用来查询班级内所有，按照学年查询
    @Query(value = "select new com.evaluation.system.domain.ExtraEntity.AllqtAwards(a.number,b.name,a.bonus_name) from qtScholarship a,basic b where a.number=b.number and a.year=?1 and b.classMajor=?2 and a.state=2")
    public List<AllqtAwards> findByYearAndClassMajorAndState(String year,String classmajor);
    ...
    @Modifying
    @Query(value = "update qtScholarship a set a.state=?1,a.reason=?5 where a.number=?2 and a.year=?3 and a.bonus_name=?4")
